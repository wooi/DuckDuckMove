...
internal static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        if (args.Length >= 2 && args[0] == "--render")
        {
            ...
            using var host = new System.Windows.Interop.HwndSource(new System.Windows.Interop.HwndSourceParameters("DuckDuckMove hidden UI verification") { Width = 804, Height = 650, WindowStyle = unchecked((int)0x80000000) });
            ...
            foreach (var (name, theme, start) in new[] { ("light-login", 1, false), ("dark-login", 2, false), ("light-start", 1, true) })
            {
                ...
                visual.Measure(new Size(804, 650)); visual.Arrange(new Rect(0, 0, 804, 650)); visual.UpdateLayout();
                Pump(250);
                var bitmap = new RenderTargetBitmap(804, 650, 96, 96, PixelFormats.Pbgra32); bitmap.Render(visual);
