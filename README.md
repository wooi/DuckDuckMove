...
当前源码为 **v0.1.2 刷新改进测试版**，新增应用后刷新开始菜单与手动刷新按钮。公开 Release v0.1.1 不含此改进；v0.1.2 的底部头像即时刷新效果仍需实机验证，Microsoft 账户卡片暂不支持同步。详见 [刷新机制与验证范围](docs/START-MENU.md)。
...
**当前为 v0.1.1 测试版，尚未进行代码签名。真实账户写入、登录界面动画和重启后的表现仍待实机验证。** 请阅读下方兼容性与验证状态；软件中的预览不代表 Windows 已应用成功。GitHub 自动生成的 Source code 压缩包是源码，不是可运行程序。
...
不选图片也能使用：启动后自动载入小鸭，点击应用即可。支持自选或拖入 GIF、浅色 / 深色主题、恢复原头像及 Windows 默认头像。
...
**恢复原头像**：恢复第一次由本软件更改前的头像，连续换图不会覆盖这份原始备份。尚未备份时按钮不可用。如果原文件已被移动或修改，使用备份图片恢复。

应用或恢复时可能每次都需要 UAC 确认。软件不会自动注销、重启或锁定电脑。设置完成后可关闭程序，临时服务会退出并被移除。

v0.1.2 默认勾选「应用或恢复后刷新开始菜单」。操作成功后会结束当前会话中的标准开始菜单宿主，让 Windows 重新加载；开始菜单会短暂关闭，请按 Win 键查看底部头像。可取消勾选，或使用「刷新开始菜单」单独重试。刷新失败不撤销已写入的头像，也不代表账户卡片会更新。
...
原头像备份与已保存素材会保留。仅删除 EXE 不会移除已设置的头像。若要清理所有数据，请先恢复原头像或默认头像，再由管理员删除本软件自己的数据目录。

## 验证状态

- 已通过：MSI 静默安装与卸载、安装文件校验、开始菜单快捷方式检查，以及安装后的应用界面测试。
- 已通过：21 项核心测试（原始备份保留、缺失键处理、默认恢复、部分写入失败、回滚失败后的恢复、读回核对、无效 GIF 等）。
- 已通过：自包含 EXE 的隐藏界面测试，验证 GIF 像素随时间变化、按钮启用、默认头像预览及重新应用预览。
- 已检查：浅色、深色和开始菜单布局。
- **尚未实测：UAC → SYSTEM 服务 → 真实账户头像写入，以及登录界面实际动画和重启后保留情况。** 此轮构建没有改变用户实际头像。
